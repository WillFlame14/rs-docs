...
- When pace > 2, it is better for the unlocked player to discard instead of playing a card that doesn't connect to one of the locked player's cards.
...
- If the unlocked player already had *LH PTD*, they do not receive additional *LH PTD*.
	- A player can have at most one card with *LH PTD*.
- The unlocked player receives *LH PTD* on every clue even if there is a commonly-known way to unlock between both players.

### Unlock Promise

- If the unlocked player plays a **commonly-known** card even when they have *Locked Hand PTD*, this promises that the locked player can become unlocked.
...
- If Alice discards slot 3 (*LH PTD*), Bob can write negative notes for all the above identities.

### Unlock Promise Shifting

- If the unlocked player delays before playing a connecting card, this causes the unlock target to shift.
