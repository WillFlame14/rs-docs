...
- To get an unclued card (the *Target*) to play, a **colour clue** is given to the unclued card immediately to its **right** (the *Focus*).
    ...
    - To get a previously-clued card to play, [reveal it as a playable](./safe-actions).
...
- The *Focus* of the clue is the leftmost **newly-clued** card, except slot 1 has the lowest precedence.
  - This means that slot 1 is never the *Focus* unless it is the only newly-clued card.
...
- Note that play clues can target newly-clued cards as well, since they were previously unclued.
